...
def main():
    ...
    checks.append(result('semantic-authority',(semantic.get('repository'),semantic.get('version'))==EXPECTED_SEMANTIC and bool(semantic.get('artifacts')),'QBF TSMM 0.24.0 must be declared with artifacts'))
    checks.append(result('schema-authority',(schema.get('repository'),schema.get('version'))==EXPECTED_SCHEMA and bool(schema.get('artifacts')),'QBF TIS 0.15.0 must be declared with artifacts'))
